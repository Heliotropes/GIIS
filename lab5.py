...
def sorty_G():
    ...
    for point in points0:
        while (points1[0][0]-points1[1][0])*(point[1]-points1[0][1])-(points1[0][1]-points1[1][1])*(point[0]-points1[0][0]) < 0:
            points1.pop(0)
...
def sorty_jarvis():
    ...
    i = 1   
    while a[i] != a[0]:
        l = sorted(l, key = lambda s:(abs((a[i][0]-a[i-1][0])*(s[0]-a[i][0]))+abs((a[i][1]-a[i-1][1])*(s[1]-a[i][1])))/(((a[i][0]-a[i-1][0])**2+(a[i][1]-a[i-1][1])**2)**0.5*((s[0]-a[i][0])**2+(s[1]-a[i][1])**2)**0.5))
        a.append(l[0])
        l.pop(0)
        i+=1
    return a
